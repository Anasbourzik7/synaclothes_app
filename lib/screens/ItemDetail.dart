...
class _ItemDetailPageState extends State<ItemDetailPage> {
  ...
  Future<void> _loadItemData() async {
    final snapshot = await database.child('clothingItems/${widget.itemId}').get(); // Récupérer les données de l'article avec l'ID spécifique
    if (snapshot.exists) {
      setState(() {
  ...
  void _addToCart(BuildContext context) {
    if (itemData != null) {
      ...
      ScaffoldMessenger.of(context).showSnackBar( // Afficher un message de confirmation que l'article a été ajouté
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: Text(itemData!['title'] ?? 'Détail article'), // Afficher le titre de l'article
      ),
      body: SingleChildScrollView(
        padding: const EdgeInsets.all(16.0), // Ajouter de l'espace autour du contenu
        child: Column(
          crossAxisAlignment: CrossAxisAlignment.start, // Alignement du texte à gauche
          children: [
            // Afficher l'image de l'article
            Image.network(
              itemData!['imageUrl'], // Utilisation de l'URL de l'image de l'article
              fit: BoxFit.cover, // Adapter l'image à l'espace disponible
              errorBuilder: (context, error, stackTrace) => 
                  const Icon(Icons.image_not_supported, size: 100), // Afficher une icône si l'image ne peut pas être chargée
            ),
            const SizedBox(height: 16.0), // Ajouter un espacement entre l'image et le texte
            ...
            Row(
              mainAxisAlignment: MainAxisAlignment.spaceBetween, // Espacer les boutons de chaque côté
              children: [
                ElevatedButton(
                ...
                ElevatedButton(
                  onPressed: () => _addToCart(context), // Ajouter l'article au panier
