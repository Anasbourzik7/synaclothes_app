...
class _LoginPageState extends State<LoginPage> {
  ...
  Future<void> _checkIfUserIsLoggedIn() async {
    final user = FirebaseAuth.instance.currentUser; // Récupérer l'utilisateur actuellement connecté
  ...
  Future<void> _signIn() async {
    if (_formKey.currentState!.validate()) { // Vérifier la validité du formulaire
      setState(() => _isLoading = true); // Afficher le chargement pendant la connexion
      
      try {
        print('Tentative de connexion avec: ${_emailController.text}'); // Message de débogage
        ...
        print('Connexion réussie: ${userCredential.user?.uid}'); // Message de débogage pour la réussite
        ...
        print('Erreur Firebase Auth: ${e.code}'); // Message de débogage pour l'erreur
        String message;
        if (e.code == 'user-not-found') { // Cas où l'utilisateur n'est pas trouvé
          message = 'Aucun utilisateur trouvé avec cet email.';
        } else if (e.code == 'wrong-password') { // Cas où le mot de passe est incorrect
        ...
        setState(() => _isLoading = false); // Masquer le chargement après la tentative de connexion
      }
    }
  }
